<template>
  <v-container fluid>
    ...
    <v-row>
      ...
      <v-col class="text-right">주문 일자 : <b>{{ this.OrderDetailInfo[0].regDati }}</b></v-col>
    </v-row>
    <v-data-table
      :headers="header"
          ...
          <v-col>
            <v-list outlined="true">
                ...
                <v-list-item>
                  ...
                  <v-list-item-subtitle>
                    {{ (String(Number(this.OrderDetailInfo[0].totDeliPri) + Number(this.OrderDetailInfo[0].totPri))).comma() }}
                    원
                  </v-list-item-subtitle>
                </v-list-item>
            </v-list>
...
export default {
  ...
  methods: {
    ...
    getProcAlert() {
      this.$dialog.error({
        title: "결제 필요",
        text: "<b>계좌로 입금이 완료되지 않았습니다.</b><br/>" +
          "입금 후 확인 부탁드립니다. <br/>" +
