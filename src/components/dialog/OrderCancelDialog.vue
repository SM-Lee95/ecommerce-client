  ...
  <v-container fluid class="white">
    <validation-observer ref="observer" v-slot="{ invalid }">
      <v-row no-gutters>
        <v-col>
          <v-row class="mt-1">
            <v-col v-if="OrderCancelInfo.procTy != '0'" class="mt-1 ml-1 text-h6">개별 주문 취소</v-col>
            <v-col v-if="OrderCancelInfo.procTy == '0'" class="mt-1 ml-1 text-h6">전체 주문 취소</v-col>
          </v-row>
          <v-divider></v-divider>
          <v-row v-if="OrderCancelInfo.procTy == '0'">
            <v-col>
              <v-row>
                <v-col class="red--text lighten-2 text-center">결제 이전 주문건은 전체 취소만 가능합니다. <br />총 {{
    OrderCancelInfo.totCnt
}} 개의 아이템 주문 건이 취소됩니다.</v-col>
              </v-row>
              <v-row>
                <v-col class="text-center">주문 요약 : {{ OrderCancelInfo.ordsDoc }}</v-col>
              </v-row>
            </v-col>
          </v-row>
          <v-row v-if="OrderCancelInfo.procTy != '0'">
  ...
  methods: {
    cancelOrder() {
      ...
      if (this.OrderCancelInfo.procTy != "0") {
        ...
        title = "개별 주문 취소 요청";
        text = "환불 받으실 계좌번호는 " + cancelObj.returnAccount + "(" + cancelObj.accountHolder + ") 입니다. \n 다시 한번 확인해주세요.";
      }
      this.$dialog
        .confirm({
          title: title,
